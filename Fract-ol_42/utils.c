...
/*   Updated: 2023/05/21 05:17:09 by ojebbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_set(t_vars *hadik)
{
	...
	hadik->addr = mlx_get_data_addr(hadik->img, &hadik->bits_per_pixel,
			&hadik->line_length, &hadik->endian);
...
void	my_mlx_pixel_put(t_vars *hadik, int x, int y, int color)
{
	...
	dst = hadik->addr + (y * hadik->line_length + x * (hadik->bits_per_pixel
				/ 8));
